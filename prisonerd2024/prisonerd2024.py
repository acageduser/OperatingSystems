        ...
        try:
            try:
                ...
                while True:         # processor runs until sys.exit.
                    ...
                    elif self.state == 'makingProcesses':
                        if event == 'fork':
                            locals["pid"] = args[0]
                            locals["tid"] = args[1]
...
class thread(_Thread_):
    ...
    def run(self):
        ...
        locals = {
            ...
            'loopCount' : 0,
            'loopLimit' : 100,
            'myLastAction' : 'cooperate',
            'otherAction' : 'cooperate',
            'otherLastAction' : 'cooperate',
            'SendRecvAction' : 'SendRecvActionBUG',
            'penalty' : 0,
        }
        self.__eventSet__ = {'init', 'cpu', 'receiveOtherAction'}
        self.locals = locals
        self.__localnames__ = locals.keys()
        self.__localcount__ = len(self.__localnames__)
                ...
                while not self.__isdead__:
                    ...
                    if self.state == 'init':
                        ...
                        if event != __lastMissedEvent__ and self.logger.getLevel() >= (__DEBUGLEVEL__-1):
                            self.msg("INFO: Missed event: " + event + " From state: " + self.state)
                            __lastMissedEvent__ = event
                    elif self.state == 'initThread':
                        if event == 'cpu':
                            if eval(__codeTable__[13],globals,locals):
                                self.logger.log(self, tag="DEPART")
                                self.state = 'sendMyAction'
                                self.logger.log(self, tag="APPROACH")
                                ...
                                continue
                        if event == 'cpu':
                            if eval(__codeTable__[19],globals,locals):
                                self.logger.log(self, tag="DEPART")
                                self.state = 'sendMyAction'
                                self.logger.log(self, tag="APPROACH")
                        ...
                        if event != __lastMissedEvent__ and self.logger.getLevel() >= (__DEBUGLEVEL__-1):
                            self.msg("INFO: Missed event: " + event + " From state: " + self.state)
                            __lastMissedEvent__ = event
                    elif self.state == 'sendMyAction':
                        if event == 'cpu':
                            if eval(__codeTable__[23],globals,locals):
                                self.logger.log(self, tag="DEPART")
                                self.state = 'awaitOtherAction'
                                self.logger.log(self, tag="APPROACH")
                        ...
                        if event == 'cpu':
                            if eval(__codeTable__[29],globals,locals):
                                ...
                                exec(__codeTable__[32],globals,locals)
                                ...
                                continue
                        if event == 'cpu':
                            if eval(__codeTable__[35],globals,locals):
                                self.logger.log(self, tag="DEPART")
                                self.state = 'awaitOtherAction'
                                self.logger.log(self, tag="APPROACH")
                                exec(__codeTable__[36],globals,locals)
                                exec(__codeTable__[37],globals,locals)
                                exec(__codeTable__[38],globals,locals)
                                exec(__codeTable__[39],globals,locals)
                        ...
                        if event != __lastMissedEvent__ and self.logger.getLevel() >= (__DEBUGLEVEL__-1):
                            self.msg("INFO: Missed event: " + event + " From state: " + self.state)
                            __lastMissedEvent__ = event
                    elif self.state == 'awaitOtherAction':
                        if event == 'receiveOtherAction':
                            locals["otherAction"] = args[0]
                            self.logger.log(self, tag="DEPART")
                            self.state = 'timeInJail'
                            self.logger.log(self, tag="APPROACH")
                        ...
                        if event != __lastMissedEvent__ and self.logger.getLevel() >= (__DEBUGLEVEL__-1):
                            self.msg("INFO: Missed event: " + event + " From state: " + self.state)
                            __lastMissedEvent__ = event
                    elif self.state == 'timeInJail':
                        if event == 'cpu':
                            if eval(__codeTable__[46],globals,locals):
                                self.logger.log(self, tag="DEPART")
                                self.state = 'sendMyAction'
                                self.logger.log(self, tag="APPROACH")
                                exec(__codeTable__[47],globals,locals)
                                self.logger.log(self, tag="ARRIVE")
                                yield None
                                __lastMissedEvent__ = None
                                continue
                        if event == 'cpu':
                            if eval(__codeTable__[48],globals,locals):
                                self.logger.log(self, tag="DEPART")
...
def main():
    ...
    sys.exit(0)
__codeTable__ = [
    compile('processor.action = [             ["defect", "defect"],             ["defect", "cooperate"],             ["defect", "halfsy"],             ["defect", "reciprocate"],             ["halfsy", "cooperate"],             ["reciprocate", "cooperate"],             ["cooperate", "cooperate"],             ["halfsy", "halfsy"],             ["halfsy", "reciprocate"],             ["reciprocate", "reciprocate"]         ]','nofile','exec'),
    compile('processor.penalty = {             "defect" : {                 "defect" : 2,                 "cooperate" : 0             },             "cooperate" : {                 "defect" : 3,                 "cooperate" : 1             }         }','nofile','exec'),
    compile('processor.processorSampling = ["defect", "cooperate"]','nofile','exec'),
    ...
    compile('msg("Example DEBUG message, pid = " + str(pid) + " tid = " + str(tid))','nofile','exec'),
    compile('mystrategy = processor.action[pid][tid]','nofile','exec'),
    compile('cpu(0)','nofile','exec'),
    compile('tid == 0','nofile','eval'),
    compile('pcb.partners = [thread, None]','nofile','exec'),
    compile('pcb.incomingMessage = ["", ""]','nofile','exec'),
    compile('othertid = 1','nofile','exec'),
    compile('spawn()','nofile','exec'),
    compile('cpu(10)','nofile','exec'),
    compile('tid == 1','nofile','eval'),
    compile('pcb.partners[1] = thread','nofile','exec'),
    compile('othertid = 0','nofile','exec'),
    compile('cpu(0)','nofile','exec'),
    compile('mystrategy == "halfsy"','nofile','eval'),
    compile('myLastAction = processor.processorSampling[sample(0,1,"uniform")]','nofile','exec'),
    compile('SendRecvAction = myLastAction','nofile','exec'),
    compile('pcb.incomingMessage[othertid] = SendRecvAction','nofile','exec'),
    compile('signalEvent(pcb.partners[othertid], "receiveOtherAction",             SendRecvAction)','nofile','exec'),
    compile("waitForEvent('receiveOtherAction', False)                 if not pcb.incomingMessage[tid]             else trigger(0, 'receiveOtherAction', pcb.incomingMessage[tid])",'nofile','exec'),
    compile('mystrategy == "defect" or mystrategy == "cooperate"','nofile','eval'),
    compile('myLastAction = mystrategy','nofile','exec'),
    compile('SendRecvAction = mystrategy','nofile','exec'),
    compile('pcb.incomingMessage[othertid] = SendRecvAction','nofile','exec'),
    compile('signalEvent(pcb.partners[othertid], "receiveOtherAction",             SendRecvAction)','nofile','exec'),
    compile("waitForEvent('receiveOtherAction', False)                 if not pcb.incomingMessage[tid]             else trigger(0, 'receiveOtherAction', pcb.incomingMessage[tid])",'nofile','exec'),
    compile('mystrategy == "reciprocate"','nofile','eval'),
    compile('myLastAction = otherLastAction','nofile','exec'),
    compile('SendRecvAction = otherLastAction','nofile','exec'),
    compile('pcb.incomingMessage[othertid] = SendRecvAction','nofile','exec'),
    compile('signalEvent(pcb.partners[othertid], "receiveOtherAction",             SendRecvAction)','nofile','exec'),
    compile("waitForEvent('receiveOtherAction', False)                 if not pcb.incomingMessage[tid]             else trigger(0, 'receiveOtherAction', pcb.incomingMessage[tid])",'nofile','exec'),
    compile('pcb.incomingMessage[tid] = None','nofile','exec'),
    compile('penalty = processor.penalty[myLastAction][otherAction]','nofile','exec'),
    compile('otherLastAction = otherAction','nofile','exec'),
    compile('loopCount += 1','nofile','exec'),
    compile('cpu(penalty)','nofile','exec'),
    compile('loopCount < loopLimit','nofile','eval'),
    compile('cpu(0)','nofile','exec'),
    compile('loopCount >= loopLimit','nofile','eval'),
    None
]
