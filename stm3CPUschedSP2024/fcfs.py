...
class thread(_Thread_):
    ...
    def run(self):
        ...
        self.__eventSet__ = {'cpu', 'yieldcpu', 'init', 'contextAvailable', 'io'}
        self.locals = locals
        self.__localnames__ = locals.keys()
        self.__localcount__ = len(self.__localnames__)
                    ...
                    if self.state == 'init':
                        ...
                        if event == 'yieldcpu':
                            if eval(__codeTable__[12],globals,locals):
                                ...
                                continue
                        if event == 'yieldcpu':
                            if eval(__codeTable__[15],globals,locals):
                                self.logger.log(self, tag="DEPART")
                                self.state = 'ready'
                                self.logger.log(self, tag="APPROACH")
...
__codeTable__ = [
    ...
    compile('processor.readyQ = Queue(False)','nofile','exec'),
    ...
    compile('tickstorun = cpuTicksB4IO','nofile','exec'),
    ...
    compile('processor.contextsFree == 0','nofile','eval'),
    compile('processor.readyQ.enq(thread)','nofile','exec'),
