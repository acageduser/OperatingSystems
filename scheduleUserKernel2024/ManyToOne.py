...
class thread(_Thread_):
    ...
    def run(self):
        ...
        self.globals = globals
        locals = {
            'cpuTicksInUserOrKernel' : 0,
            'userTicks' : 1000,
            'kernelTicks' : 500,
            'testRunsToGo' : 100,
            'region' : 0,
        }
        self.__eventSet__ = {'unlocked', 'cpu', 'init'}
        self.locals = locals
        self.__localnames__ = locals.keys()
        self.__localcount__ = len(self.__localnames__)
                ...
                while not self.__isdead__:
                    ...
                    if self.state == 'init':
                        ...
                        if event != __lastMissedEvent__ and self.logger.getLevel() >= (__DEBUGLEVEL__-1):
                            self.msg("INFO: Missed event: " + event + " From state: " + self.state)
                            __lastMissedEvent__ = event
                    elif self.state == 'userMode':
                        if event == 'cpu':
                            if eval(__codeTable__[13],globals,locals):
                                self.logger.log(self, tag="DEPART")
                                self.state = 'stallThread'
                                self.logger.log(self, tag="APPROACH")
                        ...
                        if event != __lastMissedEvent__ and self.logger.getLevel() >= (__DEBUGLEVEL__-1):
                            self.msg("INFO: Missed event: " + event + " From state: " + self.state)
                            __lastMissedEvent__ = event
                    elif self.state == 'kernelMode':
                        if event == 'cpu':
                            if eval(__codeTable__[21],globals,locals):
                                self.logger.log(self, tag="DEPART")
                                self.state = 'userMode'
                                self.logger.log(self, tag="APPROACH")
                                ...
                                continue
                        if event == 'cpu':
                            if eval(__codeTable__[26],globals,locals):
                                self.logger.log(self, tag="DEPART")
                                self.state = 'done'
                                self.logger.log(self, tag="APPROACH")
                                self.logger.log(self, tag="ARRIVE")
...
def main():
    ...
    sys.exit(0)
__codeTable__ = [
    compile('processor.KernelRegions = 1','nofile','exec'),
    compile('processor.WaitQueues = [Queue(False)             for i in range(0, processor.KernelRegions)]','nofile','exec'),
    compile('processor.KernelLock = [0 for i in range(0, processor.KernelRegions)]','nofile','exec'),
    ...
    compile('region = sample(0, processor.KernelRegions-1, "uniform")','nofile','exec'),
    compile('cpuTicksInUserOrKernel = userTicks','nofile','exec'),
    compile('cpu(cpuTicksInUserOrKernel)','nofile','exec'),
    compile('processor.KernelLock[region] == 1','nofile','eval'),
    compile('processor.WaitQueues[region].enq(self)','nofile','exec'),
    compile("waitForEvent('unlocked')",'nofile','exec'),
    compile('processor.KernelLock[region] == 0','nofile','eval'),
    compile('processor.KernelLock[region] = 1','nofile','exec'),
    compile('testRunsToGo -= 1','nofile','exec'),
    compile('cpuTicksInUserOrKernel = kernelTicks','nofile','exec'),
    compile('cpu(cpuTicksInUserOrKernel)','nofile','exec'),
    compile('testRunsToGo > 0','nofile','eval'),
    compile('processor.KernelLock[region] = 0','nofile','exec'),
    compile("if len(processor.WaitQueues[region]) > 0:             signalEvent(processor.WaitQueues[region].deq(), 'unlocked')",'nofile','exec'),
    compile('cpuTicksInUserOrKernel = userTicks','nofile','exec'),
    compile('cpu(cpuTicksInUserOrKernel)','nofile','exec'),
    compile('testRunsToGo == 0','nofile','eval'),
