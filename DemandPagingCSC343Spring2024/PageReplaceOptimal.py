                    ...
                    if self.state == 'init':
                        ...
                        if event == 'cpu':
                            if eval(__codeTable__[11],globals,locals):
                                ...
                                continue
                        if event == 'cpu':
                            if eval(__codeTable__[14],globals,locals):
                                self.logger.log(self, tag="DEPART")
                        ...
                        if event == 'cpu':
                            if eval(__codeTable__[26],globals,locals):
                                ...
                                continue
                        if event == 'cpu':
                            if eval(__codeTable__[27],globals,locals):
                                self.logger.log(self, tag="DEPART")
                                self.state = 'readyToCompute'
                                self.logger.log(self, tag="APPROACH")
...
__codeTable__ = [
    ...
    compile('victimPage = -1','nofile','exec'),
    compile('future = []','nofile','exec'),
    compile('for respage in residentPageMap:             future.append(referencePageList.index(respage)                  if (respage in referencePageList)                 else (respage * referencePageCount * 10))','nofile','exec'),
    compile('victimPage = max(future)','nofile','exec'),
    compile('victimPage = referencePageList[victimPage]             if (victimPage < len(referencePageList))             else int(victimPage / (referencePageCount * 10))','nofile','exec'),
